...
function (x, y = NULL, alpha) 
{
    ...
    if (n.arc > 0) {
        while (watch <= n.arc) {
            ...
            deltaX <- arcs[watch, 1] - arcs[seq_len(initialArcCount), 1]
            deltaY <- arcs[watch, 2] - arcs[seq_len(initialArcCount), 2]
            centreDistances <- sqrt(deltaX^2 + deltaY^2)
            canIntersect <- centreDistances > abs(arcs[watch, 3] - arcs[seq_len(initialArcCount), 3]) &
              centreDistances < arcs[watch, 3] + arcs[seq_len(initialArcCount), 3]
            ...
            while (j <= n.arc) {
                if (j != watch) {
                  if (j <= initialArcCount) {
                    ...
                    centreDistance <- sqrt(
                      (arcs[watch, 1] - arcs[j, 1])^2 +
                        (arcs[watch, 2] - arcs[j, 2])^2
                    )
                    pairCanIntersect <- centreDistance > abs(arcs[watch, 3] - arcs[j, 3]) &&
